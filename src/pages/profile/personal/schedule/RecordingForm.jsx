...
export const DownloadRecordingButton = ({ downloadUrl, playUrl, closeDialog, isStaff, index }) => {
...
const RecordingForm = ({ data, callbackFnc, isStaff }) => {
      ...
      {_.get(data, 'meeting.records', []).map((item, index) => {
        ...
        return (
          ...
          >
            ...
            <ListItemText
              primary={formatDuration(_.get(downloadItem, 'startAt'), _.get(downloadItem, 'endAt'))}
              secondary={'mp4'}
            />
          </ListItem>
        );
