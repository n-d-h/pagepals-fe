    ...
    <Stack component={Card} spacing={3} sx={{ p: 3, boxShadow: isCustomer && 'rgba(100, 100, 111, 0.2) 0px 7px 29px 0px', mr: 2 }}>
      ...
      <Stack spacing={2}>
        ...
        <Stack direction="row" alignItems="center" spacing={1}>
          {listGenre && listGenre.length > 0 && listGenre?.map((genre) => (
      ...
      <Stack spacing={2}>
        ...
        <Stack direction="row" alignItems="center" spacing={1}>
          ...
          listLanguage.map((listLanguage) => {
            const languageObj = uniqueLanguages.find((language) => language === listLanguage);
            return (
    ...
    <Stack component={Card} spacing={2} sx={{ p: 3, mb: 5, boxShadow: isCustomer && 'rgba(100, 100, 111, 0.2) 0px 7px 29px 0px' }}>
      ...
      <List>
        {[
          {
            label: 'Account ID',
            value: _.get(reader, 'readerRequestReference.account.id', 'null'),
            icon: <Iconify icon="solar:user-id-bold" width={24} />,
          },
          {
            label: 'Username',
            value: _.get(reader, 'readerRequestReference.account.username', 'null'),
            icon: <Iconify icon="solar:user-bold" width={24} />,
          },
          {
            label: 'Phone Number',
            value: _.get(reader, 'readerRequestReference.account.phoneNumber', 'null'),
            icon: <Iconify icon="solar:phone-calling-bold" width={24} />,
          },
          {
            label: 'Email',
            value: _.get(reader, 'readerRequestReference.account.email', 'null'),
