...
import { getListCustomerAndListAccountState, getListReaderAndListAccountState } from '../../../services/apolo/queries';
...
export default function UserListView () {
  ...
  const setDataFetching = (data) => {
    setTableData(data.getListReader);
  ...
  const { loading, error, data } = useQuery(getListReaderAndListAccountState, {
								...
								icon={
									<Label
										...
										color={
											...
											(tab.value === 'reader_active' && 'success') ||
											(tab.value === 'reader_pending' && 'warning') ||
											'default'
										}
									>
										...
										{tab.value === 'reader_active' &&
											tableData?.filter((user) => user.accountState.name.toLowerCase() === 'reader_active').length}
										{tab.value === 'reader_pending' &&
											tableData?.filter((user) => user.accountState.name.toLowerCase() === 'reader_pending').length}
										{tab.value === 'deleted' &&
